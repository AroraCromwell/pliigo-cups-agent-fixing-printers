...
class Printer {
    ...
    constructor(queue) {
        ...
        if(typeof queue == "string"){
            ...
            this._options = this.getOptionsForPrinter();
    ...
    getOptionsForPrinter(callback) {
        let rx_options = /([\w\-_]+)=('{1}.+?'{1}|"{1}.+?"{1}|.+?)(?:\s|$)/g;
        let cmd = `lpoptions -p ${this._queue}`;
        ...
        if(typeof callback == "function"){
            exec(cmd, (error, stdout, stderr) => {
                ...
                if (error || stderr) {
                    if (typeof callback == "function") {
                        return callback(new Error("can not load options"), null);
                    }
                }
                ...
                let tmp = stdout.match(rx_options);
                let returner = {};
                _.forEach(tmp, (line)=> {
                    line = line.replace(/\'|\"/g, "");
                    let lineSplit = line.split("=");
                    returner[lineSplit[0]] = lineSplit[1].trim();
                });
                if (typeof callback == "function") {
     ...
     * @returns {boolean}
     */
    _setsharedOnNetwork(isShared, cb) {
    ...
    uninstall(callback) {
        ...
        let cmd = `lpadmin -x ${this._queue}`;
        if(typeof callback == "function"){
            exec(cmd, (error, stdout, stderr)=>{
                if(!error && !stderr){
                    callback(null, true);
                } else {
                    callback(new Error("was not able to delete this printer"), false);
                }
            });
